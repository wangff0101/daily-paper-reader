...
tags: ["query:medllm"]
score: 6.0
evidence: 多文档信息结构化与实体关系推导
tldr: "针对多文档多实体问答（MDMEQA）中逻辑追踪难、检索不精准等问题，本文提出DocSage智能体框架。该框架通过动态模式发现、结构化信息提取及模式感知推理，将非结构化文本转化为关系表，利用SQL索引实现精准定位与跨文档实体关联。实验表明，DocSage在多项基准测试中显著优于现有长文本模型和RAG系统，准确率提升超过27%，有效解决了复杂场景下的证据链构建难题。"
selection_source: fresh_fetch
motivation: 现有RAG和长文本模型在处理跨文档复杂实体关系时，存在关键事实遗漏、缺乏模式感知及证据链构建困难等局限。
method: 提出一种集成动态模式发现、结构化提取与模式感知推理的智能体框架，通过将非结构化文本转化为关系表来增强逻辑推理能力。
result: "在两个MDMEQA基准测试中，DocSage相比最先进的长文本LLM和RAG系统，准确率提升了27%以上。"
conclusion: DocSage通过结构化信息处理有效解决了多文档实体问答中的逻辑对齐难题，为复杂信息检索与推理提供了高效的新范式。
---
