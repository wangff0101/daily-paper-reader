...
tldr: 本研究针对手术基本动作（BSA）的识别与建模，构建了包含6个专科、10类动作、逾1.1万个视频片段的目前最大规模BSA数据集。基于此开发了通用基础模型，实现了跨专科的稳健动作识别。该模型在手术技能评估及基于视觉语言模型（VLM）的手术规划中表现出色，经多国外科医生评估具有临床相关性，为实现手术超级智能奠定了基础。
selection_source: fresh_fetch
motivation: 旨在通过理解和建模手术基本动作这一核心单元，推动手术实践、培训及自动化的智能化演进。
method: 构建了最大规模的跨专科BSA数据集，并开发了一个能够进行通用手术动作识别的基础模型。
result: 该模型在不同手术类型和身体部位的跨专科实验中表现稳健，并成功应用于技能评估和可解释的手术规划。
conclusion: 准确的BSA理解模型能有效促进复杂临床应用，加速手术人工智能向更高层级的发展。
---
