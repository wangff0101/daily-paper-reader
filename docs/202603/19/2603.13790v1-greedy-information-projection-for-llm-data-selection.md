...
method: 通过最大化查询信号与数据子集间的互信息，利用贪婪匹配追踪算法将查询嵌入投影到数据张成空间进行选择。
result: 在指令遵循和数学推理实验中，GIP仅利用极少比例的数据便实现了与全量数据微调相当的性能。
conclusion: GIP提供了一个理论严密且计算高效的框架，成功实现了质量与多样性在数据选择中的自然平衡。
---

## 摘要
我们提出了贪婪信息投影（Greedy Information Projection，简称 GIP），这是一个用于为大语言模型微调选择训练样本的有原则的框架。GIP 将选择过程建模为最大化样本子集与特定任务查询信号之间的互信息，这些信号可能源自大语言模型的质量判断、元数据或其他来源。该框架涉及优化一个使用数据和查询嵌入定义的闭式互信息目标，从而自然地平衡了“质量”与“多样性”。优化该得分等同于最大化查询嵌入矩阵在所选数据张成空间上的投影，这为质量与多样性的共同出现提供了几何解释。基于这一观点，我们采用了一种快速的贪婪匹配追踪程序，并结合了高效的基于投影的更新。在指令遵循和数学推理数据集上，GIP 仅使用一小部分样本和计算资源，就能选择出与全量数据微调效果相当的小规模子集，将质量感知和多样性感知选择统一起来，实现了高效微调。
