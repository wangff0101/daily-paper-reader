...
tldr: 本研究探讨了视觉语言模型（VLM）中由于“锥状效应”导致的模态间隙对监督多模态学习的影响，特别是在医疗领域。作者提出了一种轻量级的事后处理机制，通过单一超参数控制跨模态分离程度，无需重新训练即可系统分析间隙对性能的作用。实验表明，适度缩小模态间隙可显著提升医疗任务表现，但完全消除间隙并非最佳，证明了模态间隙是多模态表示中一个关键的可调节属性。
selection_source: fresh_fetch
motivation: 旨在探究视觉语言模型中普遍存在的模态间隙现象对下游监督多模态学习（尤其是医疗领域）的实际影响。
method: 引入一种轻量级事后处理机制，通过超参数控制冻结编码器的跨模态分离程度，实现对模态间隙的系统化调节。
result: 实验发现缩小过大的模态间隙能提升性能且医疗数据对此更敏感，但完全消除间隙并非最优，最佳表现取决于任务相关的适度分离。
conclusion: 模态间隙不应被普遍最小化，而应被视为多模态表示中一个可根据具体任务进行优化的可调属性。
---
