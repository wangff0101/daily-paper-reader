...
date: 2026-03-15
pdf: "https://arxiv.org/pdf/2603.14177v1"
tags: ["query:medllm"]
score: 7.0
evidence: 用于非侵入性高钾血症检测的AI心电图系统
...
result: "系统在内外验证中均表现出高准确度，外部验证阴性预测值超99.3%，且对中重度高钾血症识别效果更佳。"
conclusion: Pocket-K系统证明了单导联AI-ECG在无创高钾血症筛查中的有效性，并成功实现了手持设备的部署原型。
---

## 摘要
高钾血症是一种危及生命的电解质紊乱，在慢性肾脏病和心力衰竭患者中十分常见，但在医院环境外进行频繁监测仍具挑战。我们开发并验证了 Pocket-K，这是一种基于 ECGFounder 基础模型初始化的单导联 AI-ECG 系统，用于无创高钾血症筛查和手持式部署。在这项利用常规临床心电图和实验室数据开展的多中心观察性研究中，共纳入 34,439 名患者的 62,290 对心电图-血钾匹配数据。研究使用 I 导联数据对模型进行了微调。北京大学人民医院的数据被分为开发集和时间验证集，天津医科大学第二医院的数据则作为独立的外部验证集。高钾血症定义为静脉血清钾 > 5.5 mmol/L。Pocket-K 在内部测试中的 AUROC 达到 0.936，在时间验证中为 0.858，在外部验证中为 0.808。对于 KDIGO 定义的中重度高钾血症（血清钾 ≥ 6.0 mmol/L），时间验证集和外部验证集的 AUROC 分别提升至 0.940 和 0.861。外部验证的阴性预测值超过 99.3%。在血钾低于高钾血症阈值的患者中，模型预测的高风险在慢性肾脏病和心力衰竭患者中更为普遍。手持原型机实现了近实时推理，为未来在原生手持和可穿戴场景下的前瞻性评估提供了支持。
