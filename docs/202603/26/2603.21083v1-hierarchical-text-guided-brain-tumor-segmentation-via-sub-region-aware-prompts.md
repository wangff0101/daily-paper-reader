...
tldr: 针对脑肿瘤分割中子区域边界模糊及现有模型忽略临床文本差异的问题，本文提出TextCSP框架。该框架采用分层文本引导策略，通过软级联解码器按解剖包含关系逐步预测，并利用子区域感知提示微调和通道调制器，将临床描述转化为特定区域的特征增强信号。在TextBraTS数据集上，该方法显著提升了Dice和HD95指标，优于现有SOTA模型。
selection_source: fresh_fetch
motivation: 现有方法通常将放射学报告压缩为单一全局嵌入，忽略了脑肿瘤不同子区域独特的临床特征和解剖包含关系。
method: 提出TextCSP框架，包含按解剖层次预测的软级联解码器、基于LoRA和软提示的子区域感知文本编码器，以及将文本语义转化为特征增强信号的通道调制器。
result: "在TextBraTS数据集上，该方法在Dice和HD95指标上分别比现有最先进方法提升了1.7%和6%。"
conclusion: 通过引入层次化文本引导和子区域感知提示，能够更有效地利用临床文本信息，显著提高脑肿瘤多区域分割的准确性。
---
